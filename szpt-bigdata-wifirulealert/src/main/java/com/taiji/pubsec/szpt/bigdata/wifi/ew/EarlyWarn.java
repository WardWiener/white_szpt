 ...
 * 
 */
package com.taiji.pubsec.szpt.bigdata.wifi.ew;
...
import scala.Tuple2;
import StayOnPlaceReduceFunction.StayRecordMapFunction;
...
import com.taiji.pubsec.szpt.bigdata.wifi.ew.basedplace.BasedPlaceCalcHelper;
import com.taiji.pubsec.szpt.bigdata.wifi.ew.basedplace.PlaceStatistics;
import com.taiji.pubsec.szpt.bigdata.wifi.ew.bean.StaysOnOnePlaceHolder;
import com.taiji.pubsec.szpt.bigdata.wifi.ew.common.StayOnPlaceMapFunction;
import com.taiji.pubsec.szpt.bigdata.wifi.ew.common.StayOnPlaceReduceFunction;
import com.taiji.pubsec.szpt.bigdata.wifi.ew.inoutnumber.InoutRecordMapFunction;
import com.taiji.pubsec.szpt.bigdata.wifi.ew.inoutnumber.InoutRecordReduceFunction;
import com.taiji.pubsec.szpt.bigdata.wifi.ew.inoutnumber.bean.InoutNumOnEveryPlaceInfo;
import com.taiji.pubsec.szpt.bigdata.wifi.ew.meets.MeetsOnOnePlaceMapFunction;
import com.taiji.pubsec.szpt.bigdata.wifi.ew.meets.MeetsOnPlacesReduceFunction;
import com.taiji.pubsec.szpt.bigdata.wifi.ew.meets.MeetsOnPlacesMapFunction;
import com.taiji.pubsec.szpt.bigdata.wifi.ew.meets.bean.MeetsOnOnePlaceHolder;
...
public class EarlyWarn {
	...
	private final static String topic = DataContants.FIVECOLORDATA;
	public final static String TOPIC_ALERT_RULE5 = "alert_rule5";
	public final static String TOPIC_ALERT_RULE6 = "alert_rule6";
			...
			new Function2<List<WifiData>, Optional<WifiData>, Optional<WifiData>>() {
				...
				public Optional<WifiData> call( List<WifiData> wifidatas, Optional<WifiData> oldState) throws Exception {
					...
					for (WifiData wifidata : wifidatas) {
						System.out.println("打印数据 ：" + wifidata);
						//沒有采集时间则跳过, 无效数据
						if (StringUtils.isEmpty(wifidata.getGatherTime())) {
							continue;
						}
						...
						if (current == null || StringUtils.isEmpty(current.getMac())) {
							...
							// 只与采集时间在状态记录之后的记录比较
							if (Long.valueOf(current.getGatherTime()) <= Long.valueOf(wifidata.getGatherTime())) {
