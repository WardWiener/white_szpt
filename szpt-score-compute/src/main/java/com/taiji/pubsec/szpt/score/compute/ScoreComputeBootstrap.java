package com.taiji.pubsec.szpt.score.compute;
 ...
 *
 */
public class ScoreComputeBootstrap {
	private static final Logger LOGGER = LoggerFactory.getLogger(ScoreComputeBootstrap.class) ;
    
    private static String[] configpaths = new String[]{"classes/applicationContext*.xml"} ;
    
	private ScoreComputeBootstrap() {
	}
    ...
    private static void generatePid(){
    	try{
    		String runClassPath = ScoreComputeBootstrap.class.getClassLoader().getResource("").getPath() ;
