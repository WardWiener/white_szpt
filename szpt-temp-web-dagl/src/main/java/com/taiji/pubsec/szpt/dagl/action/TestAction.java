...
public class TestAction extends LoginInfoAction{
	...
	//保存图片附件
		public String saveImg(){
			...
			List<String> fileBeanLst = (List<String>) rqst.get("fileBeanLst");
			for (int i = 0; i < fileBeanLst.size(); i++) {
//				FileBean file = (FileBean) JSONObject.toBean(fileBeanLst.get(i), FileBean.class);
				Attachment att = attachmentCustomizedService.findById(fileBeanLst.get(0).toString());
				att.setTargetId("130412199001233418");// id是业务对象的id
