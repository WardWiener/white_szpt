	...
	$(document).ajaxSend(function(e,xhr,opt){
		...
		if($.globalSettings.ajaxLoading){
			if(!$.util.exist(opt.customizedOpt)){
				...
				if($.globalSettings.layerAjaxLoadingFrameParam == "top"){
					$.util.topWindow().$.layerAlert.loading({
						zIndex : 999999999
					}) ;
				}else{
					$.util.topWindow().frames[$.globalSettings.layerAjaxLoadingFrameParam].$.layerAlert.loading({
						zIndex : 999999999
					}) ;
				}				
			}
		...
		if($.globalSettings.ajaxLoading){
			if(!$.util.exist(opt.customizedOpt)){
				...
				if($.globalSettings.layerAjaxLoadingFrameParam == "top"){
					$.util.topWindow().$.layerAlert.closeAll('loading'); 
				}else{
					$.util.topWindow().frames[$.globalSettings.layerAjaxLoadingFrameParam].$.layerAlert.closeAll('loading'); 
				}				
			}
		}			
...
//			$.validform.initValidform() ;
		...
		frameWindow:function(selector){
			return $.util.topWindow().$(selector)[0].contentWindow ;
		},
...
/*				if($.util.exist(a_id)){
		...
		openWindow:function(url, title, width, height, top, left){
			 ...
			 * 打开新的window对象
			 */
			var w = $($.util.topWindow()).width() ;
			var h = $($.util.topWindow()).height() ;	
			...
			$.util.topWindow().open(url, title, settings) ;			
		},
