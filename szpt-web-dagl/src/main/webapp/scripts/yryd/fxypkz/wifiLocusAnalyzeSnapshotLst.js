$.wifiLocusAnalyzeSnapshotLst = $.wifiLocusAnalyzeSnapshotLst || {};
...
(function($){
	...
	$(document).on("click","#wifiLocusAnalyzeTable tr",function(){
		var rowData = $(this).data("data");
		lookWifiLocusSnapshotDetail(rowData);
	})
	...
	function init(){
		...
		$.ajax({
			url:context +'/yrydSnapshot/findWifiLocusSnapshotLst.action',
			type:"post",
			data :dataMap,
			...
			success:function(successDate){
				if(successDate.resultMap.result.length > 1){
					$("#wifiLocusAnalyzeLst").show();
					$.dagl.yryd .yrydHighriskPersonAlert.createTrackAnalyzeLstTable("wifiLocusAnalyzeTable",successDate.resultMap.result)
				}else if(successDate.resultMap.result.length == 1){
					$("#wifiLocusAnalyzeInfo").show();
					initWifiLocusInfo(successDate.resultMap.result[0].id)
				}else{
					$("#nullWifiLocusSnapshotDiv").show();
				}
			}
		});
	}
	
	function initWifiLocusInfo(id){
		var data = {
				"id":id
		}
		var dataMap = $.util.toJSONString(data);
		$.ajax({
			...
			success:function(successDate){
				...
				$.wifiLocusMap.newPlaceInfoPoint(wifiLocusMapConten,wifiPlaceInAndOutInfoBeanList);
				$.wifiLocusMap.createHotMap(wifiLocusHotMapConten,wifiTrackHotLayer,wifiPlaceInAndOutInfoBeanList);
				//设置保存快照原因
	...
	function lookWifiLocusSnapshotDetail(rowData){
		
		window.top.$.layerAlert.dialog({
			content : context +  '/yrydSnapshot/showWifiLocusAnalyzeDetail.action',
			pageLoading : true,
			title:"Wifi轨迹分析快照详情",
			width : "1000px",
			height : "650px",
	...
	function setMaxTimePlaceOrderList(hitSumBeanList){
		...
		$.each(hitSumBeanList,function(w,hitSumBean){
			...
			$("<button>",{
				"class" : "btn btn-xs btn-bordered fi-ceng-out-but",
			//	"text" : millisecondToDate(hitSumBean)
			}).appendTo(tdDiv);
			//详细时间表div
			var timeInfoDiv = $("<div>",{
				"class" : "fi-ceng alert-info"
	 ...
	 * 暴露本js方法，让其它js可调用
	 */
	jQuery.extend($.wifiLocusAnalyzeSnapshotLst, { 
		wifiLocusLstInit:init
	});	
})(jQuery);