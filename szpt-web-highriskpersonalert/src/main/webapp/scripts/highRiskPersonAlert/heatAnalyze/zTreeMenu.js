...
(function($){
	...
	function init(domId, url, customSetting, zTreeSetting ,code){
		...
		if(customSetting && customSetting.async.enable){
			...
			setting.check.chkboxType = customSetting.check.chkboxType;
			setting.async.url = url;
//			setting.async.otherParam = customSetting.async.requestData;
			setting.async.otherParam = code;
			if(customSetting.callback!=null){
				setting.callback = customSetting.callback;
			}
	...
	function getCheckedValue(bindedDomIdOrZtreeId){
		...
		return {"showNames":showNames, "ids":ids, "data":data, "codes" : codes} ;
	}
