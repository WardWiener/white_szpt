	...
	$(document).ready(function() {
		...
		$(document).on("click","#importExcel",function(){
			window.top.$.layerAlert.dialog({
				content : context +  '/personDetail/showImportExcelPage.action',
				pageLoading : true,
				title:"导入高危人",
				width : "450px",
				height : "300px",
		...
		$(document).on("click",".detail",function(){
			...
			window.location.href = context +"/personDetail/showPersonDetailPage.action?clickOrder=3&&clickListOrder=0&&idcode=" + idcode;
		});
		...
		$(document).on("click",".heatAnalyze",function(){
			...
			var form = $.util.getHiddenForm(context +'/heatAnalyze/jumpHeatAnalyzePage.action?clickOrder=4&&clickListOrder=0',data);
			$.util.subForm(form);
			...
			},
			{
				...
				"render" : function(data, type, full, meta) {
					...
					if(full.operateStatusName==""||full.operateStatus==$.common.Constant.CZZT_SPTG()){
						...
						a+='<button class="btn btn-bordered btn-xs approve" idcode="'+data+'">审批</button>';
