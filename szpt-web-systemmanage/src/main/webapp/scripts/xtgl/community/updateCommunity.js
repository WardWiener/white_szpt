(function($){
	...
	var initData = frameData.initData.community;
	 ...
	 */
	function valuation(){
		$("#communityName").val(initData.name);
		$("#communityCode").val(initData.code);
		$("#unitId").val(initData.unitId);
		$("#unitName").val(initData.unitName);
		 $("#bigCommunity").select2("val", initData.bigCommunityId); 
	}
	...
	function updateCommunity(){
		...
		var flag = $.validform.check(demo) ;     //判断是否有没有通过的验证
		  ...
		  dataMap["commBean.id"] = initData.id;
		  $.ajax({
	            url: context + '/community/uodateCommunity.action',
	            type:"POST",  
	            dataType:"json",
	            data:dataMap,
	            success:function(json){
