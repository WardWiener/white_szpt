...
(function($){
	 ...
	 * 初始化页面案件类别相关字典项
	 */
	function initCaseTagAjlb(type, firstSort, secondSort){
		var dicItemSettings = {
			id:"caseTagAjlb",
	...
	function init(){
		...
		$.ajax({		   
			...
			success:function(obj){
				...
				for( var i in td){
					$("#zatd").append('<p class="col-xs-6"><input id="zatd'+td[i].code+'"type="checkbox" style="margin: 3px 3px 5px 5px;" name="featureCodes" class="form-val icheckbox" value="'+td[i].code+'"><label for="zatd'+td[i].code+'">' +td[i].name+'</label></p>')
				}
				
				queryCaseTagByCaseCode();
			}
		});
	}
	 ...
	 * 根据案件编号查询打标
	 */
	function queryCaseTagByCaseCode(){
		if($.util.isBlank(id)){
			return ;
		}
		//查询案件分析打标信息
		var data = {"caseCode" : id};
		...
		$.ajax({
			...
			success:function(successData){
				setCaseTagInfo(successData.resultMap.ctb)
			}
		});
	}
	...
	function setCaseTagInfo(obj){
		initCaseTagAjlb(obj.type, obj.firstSort, obj.secondSort);
		$.validform.setFormVals("#tagValidform", obj);
		latitude = obj.latitude;
		longitude = obj.longitude;
		//设置作案特点
		$.each(obj.featureCodes,function(i,val){
	...
	function saveOrUpdate(){
		...
		var featureCodes = tagObj.featureCodes;
